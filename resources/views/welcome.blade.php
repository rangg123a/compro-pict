...
<section class="py-20 bg-slate-50 relative overflow-hidden" data-aos="fade-up">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 pb-4 border-b border-slate-200 gap-4">
            <div>
                <h2 class="text-3xl sm:text-4xl font-extrabold text-slate-900 mt-1">Shipping Route Network</h2>
            </div>
            <p class="text-slate-600 text-sm max-w-md">Patimban Port is seamlessly linked to domestic feeder channels and international deep-sea lanes connecting major Asian automotive hubs.</p>
        </div>    
        
        <div class="bg-white border border-slate-200 rounded-2xl p-2 sm:p-6 shadow-sm">
        
        <p class="md:hidden text-xs text-slate-500 font-medium mb-3 mt-2 flex justify-center items-center gap-1.5 opacity-80">
            <span class="text-red-600">&larr;</span> Scroll map to explore routes <span class="text-red-600">&rarr;</span>
        </p>
        ...
        <!-- Route Legends -->
        <style>
            #routeLegend .route-legend-btn.is-active,
            #routeLegend .route-legend-btn.is-active:hover,
            #routeLegend .route-legend-btn.is-active:focus-visible {
                background-color: var(--active-route-color) !important;
                color: var(--active-route-text) !important;
            }
            #routeLegend .route-legend-btn[data-route="all"] { --active-route-color: #dc2626; --active-route-text: #ffffff; }
            #routeLegend .route-legend-btn[data-route="international"] { --active-route-color: #d97706; --active-route-text: #ffffff; }
            #routeLegend .route-legend-btn[data-route="domestic"] { --active-route-color: #0284c7; --active-route-text: #ffffff; }
        </style>
        <div id="routeLegend" class="flex flex-wrap gap-2 justify-center mt-2 sm:mt-6 pt-3 sm:pt-5 border-t border-slate-200">
            <button type="button" data-route="all" class="route-legend-btn is-active flex items-center gap-2 text-xs font-semibold text-slate-700 rounded-full px-3 py-2 hover:bg-slate-100 transition"><span class="w-2.5 h-2.5 rounded-full bg-red-600 inline-block"></span>All Routes</button>
            <button type="button" data-route="international" class="route-legend-btn flex items-center gap-2 text-xs font-semibold text-slate-700 rounded-full px-3 py-2 hover:bg-slate-100 transition"><span class="w-2.5 h-2.5 rounded-full bg-amber-600 inline-block"></span>International</button>
            <button type="button" data-route="domestic" class="route-legend-btn flex items-center gap-2 text-xs font-semibold text-slate-700 rounded-full px-3 py-2 hover:bg-slate-100 transition"><span class="w-2.5 h-2.5 rounded-full bg-sky-600 inline-block"></span>Domestic</button>
        </div>

        <!-- Route Info Panel -->
        <div id="routeInfoPanel" class="mt-4 rounded-xl border-l-4 border-red-600 bg-slate-50 px-4 py-3 text-sm text-slate-700 leading-relaxed transition-colors border border-slate-200 shadow-sm">
            <p id="routeInfoTitle" class="font-bold text-slate-900 mb-0.5">Patimban Port Network</p>
            ...
            <div class="mt-3 grid gap-2 sm:grid-cols-2" aria-label="Route details">
                <div class="rounded-lg border border-amber-400/40 bg-amber-50/60 px-3 py-2">
                    <p class="font-bold text-amber-800">International Routes</p>
                    <p class="mt-1 text-xs text-slate-600 leading-relaxed">
                        <span class="font-semibold text-amber-900">Export:</span> Port Klang, Laem Chabang, Hong Kong, Guangzhou, Nagoya, Yokohama, Osaka.
                        <span class="font-semibold text-amber-900"> Import:</span> Tahara, Batangas, Muara, Singapore.
                    </p>
                </div>
                <div class="rounded-lg border border-sky-400/40 bg-sky-50/60 px-3 py-2">
                    <p class="font-bold text-sky-800">Domestic Routes</p>
                    <p class="mt-1 text-xs text-slate-600 leading-relaxed">
